          ...
          <form method="GET" action="/reportes_indicadores" enctype="multipart/form-data">
            ...
            <div class="form-group row">
              <div class="col col-lg-3 col-md-3 col-sm-12 col-xs-12">
                ...
                <select id="selectUN" required="required" name="selectUN" class="form-control" autofocus="autofocus">
                  ...
                  @foreach($unidades as $unid) 
                    @if($unid->COD_EMPRESA == $id_empresa)
                    <option  style="font-size: 90%" value="{{ $unid->COD_EMPRESA }}" {{ old('selectUN')==$unid->COD_EMPRESA ? 'selected' : '' }}selected>{{ $unid->NOMBRE_EMPRESA }}</option>
                    @else
              ...
              <div class="col col-lg-3 col-md-3 col-sm-12 col-xs-12">
                ...
                <select id="selectProy" required="required" name="selectProy" class="form-control" autofocus="autofocus">
                  <option value="-1">Seleccione Proyecto</option>
                  @foreach($proyectos as $proy)
                    @if(count($proyecto_seleccionado) != 0)
                    @foreach($proyecto_seleccionado as $proy_selec)
                    @if($proy->id_proyecto == $proy_selec->id_proyecto)
                      <option  value="{{ $proy_selec->id_proyecto }}  {{ old('selectProy') == $proy_selec->id_proyecto ? 'selected' : '' }}" selected>{{ $proy_selec->descripcion_proyecto }}</option>
                    @else
                      <option  value="{{ $proy->id_proyecto }}  {{ old('selectProy') == $proy->id_proyecto ? 'selected' : '' }}" >{{ $proy->descripcion_proyecto }}</option>
                    @endif
                    @endforeach
                    @else
                      <option  value="{{ $proy->id_proyecto }}  {{ old('selectProy') == $proy->id_proyecto ? 'selected' : '' }}" >{{ $proy->descripcion_proyecto }}</option>
              ...
              <!--<div class="col col-lg-3 col-md-3 col-sm-12 col-xs-12">
                <label for="selectTP" style="display: none">Tipo Proyecto</label>
                <select id="selectTP" required="required" style="display: none" name="selectTP" class="form-control" autofocus="autofocus">
                  <option value="-1" selected>Seleccione Tipo Proyecto</option>
                </select>
              </div>-->
              <div class="col col-lg-3 col-md-3 col-sm-12 col-xs-12 " style="padding-top:  1.7%">
                <button  class="btn btn-danger" style="color:#ffffff"><i class="far fa-save"></i> GENERAR REPORTES</button>
              </div>
            </div>
          </form>
        </div>    
      ...
      <div class="form-grou row">
        <div class="col col-lg-12 col-md-12">
          <div class="box">
            ...
            <div class="box-body">
              @if(count($valoresAccidentabilidad) !=0)
...
@section('scripts')
    <script type="text/javascript" src="{{ asset('js-proyect/admin/proyectos/reportes_search.js') }}"></script>
    ...
    @if(count($valoresTE) != 0 && count($valoresTFSA) != 0 && count($valoresTFCA) != 0 && count($valoresGravedad) != 0 && count($valoresAccidentabilidad) != 0)
